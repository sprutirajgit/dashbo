...
public class Comp30days {
	static Logger log = LogManager.getLogger(Comp15days.class);
	@Test(priority=2)
	public void CompleteExam30days(Method method) throws InterruptedException {
		System.out.println("\n Welcome \n");
	     ...
	     WebDriverManager.chromedriver().setup();
			...
			Reporter.log( "Maximize the Browser", true );
	         ...
	         System.out.println("The number of Record  is from Portal on 30 days Filletr "+ row.size());
	         log.info("The number of Record  is from Portal on 30 days Filletr "+ row.size());
	 		...
	 		Reporter.log("Login to the WorkBench",true);
	 		 driver.findElement(By.xpath("/html/body/div[1]/div[1]/div/div/div[2]/div[3]/form/div[1]/div/input[1]")).sendKeys(" implementations@ken42.spj.playground");
	 			...
	 			Reporter.log("Clear the Extra API On the URI ",true);
	 			 driver.findElement(By.xpath("/html/body/div[2]/form/input[2]")).sendKeys("/services/apexrest/ExamData/?completedIn=30");
	 			  log.info("on the api Testing Here We fillter the Exam Complete the 30 days");
	 			 Reporter.log("on the api Testing Here We fillter the Exam Complete the 30 days ",true);
